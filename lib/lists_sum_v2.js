...
function calculateField (field) {
  ...
  if (columns[field.dataset.name]) {
    ...
    for (var n in tr) {
      ...
      if (td && td[number]) {
        ...
        if (parse) {
          price = parseFloat(parse[2].replace(/\,/g, ''));
          unit = parse[1] || parse[3];
          if (price) {
            if (unit === '$') price *= courses['USD/KZT'];
            else if (unit === '€') price *= courses['EUR/KZT'];
            else if (unit === 'руб.') price *= courses['RUB/KZT'];
            else if (!unit) price *= courses['GBP/KZT'];
          }
          total += price;
        }
      }
  ...
  field.title = field.dataset.total = total.toFixed(2).toString() + ' тг.';
}
